...
namespace ImageFlip
{
    internal class Program
    {
        ...
        private static void Main(string[] args)
        {
            ...
            do
            {
                ...
                Console.WriteLine("BlockId ({0}) pixelFormat ({1}) imageFilpType({2}), Start image flip...",
                    frame.frameInfo.blockId, frame.frameInfo.pixelFormat, (IMVFGDefine.IMV_FG_EFlipType) imageFlipType);

                // 图片翻转
                // Image flip
                imageFlip(frame, (IMVFGDefine.IMV_FG_EFlipType) imageFlipType);
        ...
        // 图片翻转
        // Image flip
        private static void imageFlip(IMVFGDefine.IMV_FG_Frame frame, IMVFGDefine.IMV_FG_EFlipType imageFlipType)
        {
            IMVFGDefine.IMV_FG_PixelConvertParam stPixelConvertParam = new IMVFGDefine.IMV_FG_PixelConvertParam();
            IMVFGDefine.IMV_FG_FlipImageParam stFlipImageParam = new IMVFGDefine.IMV_FG_FlipImageParam();
            ...
            else
            {
                ...
                if (res == IMVFGDefine.IMV_FG_OK)
                {
                    ...
                    stFlipImageParam.ePixelFormat = IMVFGDefine.IMV_FG_EPixelType.IMV_FG_PIXEL_TYPE_BGR8;
                    nChannelNum = BGR_CHANNEL_NUM;
                }
                else
                {
            ...
            do
            {
                ...
                res = card.IMV_FG_FlipImage(ref stFlipImageParam);
                if (res == IMVFGDefine.IMV_FG_OK)
                {
                    if (imageFlipType == IMVFGDefine.IMV_FG_EFlipType.IMV_FG_TYPE_FLIP_VERTICAL)
                    {
