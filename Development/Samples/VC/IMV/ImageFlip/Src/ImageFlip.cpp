...
// 图片翻转
// Image flip
static void imageFlip(IMV_HANDLE devHandle, IMV_Frame frame, IMV_EFlipType imageFlipType)
{
	...
	else
	{
		...
		if (IMV_OK == ret)
		{
			...
			stFlipImageParam.ePixelFormat = gvspPixelBGR8;
			nChannelNum = BGR_CHANNEL_NUM;
		}
		else
		{
	...
	do
	{
		...
		ret = IMV_FlipImage(devHandle, &stFlipImageParam);
		if (IMV_OK == ret)
		{
			if (typeFlipVertical == imageFlipType)
			{
...
static IMV_EFlipType selectImageFlipType(void)
{
	char inputStr[256];
	char* pTrimStr;
	int inputIndex = -1;
	...
	return (IMV_EFlipType)inputIndex;
}
...
int main()
{
	...
	IMV_Frame frame;
	IMV_EFlipType imageFlipType = typeFlipVertical;
	...
	do
	{
		...
		// 图片翻转
		// Image flip
		imageFlip(devHandle, frame, imageFlipType);
