...
/// \~chinese
/// \brief 拉流处理示例
/// \example GrabStrategyUpcomingImage.cpp
/// \~english
...
bool g_isGetFrameRequest = false;
...
unsigned __stdcall softTriggerProc(void* pUserData)
{
	int ret = IMV_OK;
	IMV_HANDLE devHandle = (IMV_HANDLE)pUserData;
	if (NULL == devHandle)
	...
	while (!g_isExitThread)
	{
		...
		// 等待获取帧请求
		// wait for getting frame request
		Sleep(100);
...
int main()
{
	...
	HANDLE frameThreadHandle = NULL;
	HANDLE triggerThreadHandle = NULL;

	// 发现设备(等待获取下一张图片仅对GigE相机有效)
	// discover camera 
	...
	do
	{
		...
		// 设置驱动包间隔时间
		// Set packet timeout
		ret = IMV_GIGE_SetInterPacketTimeout(devHandle, 50);
		if (IMV_OK != ret)
		{
			break;
		}
		...
		// 开始拉流(等待获取下一张图片)
		// Start grabbing(Waiting for next image(GigE only))
		ret = IMV_StartGrabbingEx(devHandle, 0, grabStrartegyUpcomingImage);
		if (IMV_OK != ret)
		{
		...
		frameThreadHandle = (HANDLE)_beginthreadex(NULL,
			0,
			frameGrabbingProc,
			(void*)devHandle,
			CREATE_SUSPENDED,
		...
		if (!frameThreadHandle)
		{
			printf("Failed to frame grabbing thread!\n");
			break;
		}

		// 创建软触发线程 
		// Create soft Trigger thread
		triggerThreadHandle = (HANDLE)_beginthreadex(NULL,
			0,
			softTriggerProc,
			(void*)devHandle,
			CREATE_SUSPENDED,
		...
		if (!triggerThreadHandle)
		{
			printf("Failed to create soft trigger thread!\n");
			break;
		}

		ResumeThread(frameThreadHandle);

		ResumeThread(triggerThreadHandle);
		...
		WaitForSingleObject(frameThreadHandle, INFINITE);
		CloseHandle(frameThreadHandle);

		WaitForSingleObject(triggerThreadHandle, INFINITE);
		CloseHandle(triggerThreadHandle);
