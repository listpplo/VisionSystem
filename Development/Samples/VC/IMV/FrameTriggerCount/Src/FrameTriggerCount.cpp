...
static void onGetFrame(IMV_Frame* pFrame, void* pUser)
{
	...
	ret = IMV_GetIntFeatureValue(devHandle, "FrameTriggerCount", &frameTriggerCount);
	if (IMV_OK != ret)
	{
	...
	ret = IMV_GetIntFeatureValue(devHandle, "FrameTriggerLostCount", &frameTriggerLostCount);
	if (IMV_OK != ret)
	{
...
static int setSoftTriggerConf(IMV_HANDLE devHandle)
{
	...
	// 触发计数清0 
	// Clear trigger count
	ret = IMV_ExecuteCommandFeature(devHandle, "FrameTriggerCountReset");
	if (IMV_OK != ret)
	{
