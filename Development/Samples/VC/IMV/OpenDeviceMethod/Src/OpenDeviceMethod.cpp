...
int main()
{
	...
	do
	{
		std::string userId = deviceInfoList.pDevInfo[cameraIndex].cameraName;
		if (!userId.empty())
		{
			// 创建设备句柄(使用自定义名)
			// Create Device Handle(by user id)
			ret = IMV_CreateHandle(&devHandle, modeByDeviceUserID, (void*)userId.c_str());
			if (IMV_OK != ret)
			{
			...
			printf("open camera device by user id.\r\n");
			// 打开相机 
			// Connect to camera 
		...
		// 创建设备句柄(使用相机唯一键值)
		// Create Device Handle(by key)
		ret = IMV_CreateHandle(&devHandle, modeByCameraKey, deviceInfoList.pDevInfo[cameraIndex].cameraKey);
		if (IMV_OK != ret)
		{
			printf("Create devHandle failed! ErrorCode[%d]\n", ret);
			break;
		...
		printf("open camera device by camera key.\r\n");
		// 打开相机 
		// Connect to camera 
		ret = IMV_Open(devHandle);
		if (IMV_OK != ret)
		...
		ret = IMV_CreateHandle(&devHandle, modeByIndex, &cameraIndex);
		if (IMV_OK != ret)
		{
			printf("Create devHandle failed! ErrorCode[%d]\n", ret);
			break;
