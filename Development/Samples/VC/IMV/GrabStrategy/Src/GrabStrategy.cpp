...
// 选择拉流策略
// Select grab strategy
static IMV_EGrabStrategy selectGrabStrategy(IMV_HANDLE devHandle)
{
	char inputStr[256];
	char* pTrimStr;
	int inputIndex = -1;
	int ret = -1;
	int strategyCnt = 0;
	IMV_DeviceInfo dvInfo;
	int nRet = IMV_OK;
	char* find = NULL;

	nRet = IMV_GetDeviceInfo(devHandle, &dvInfo);
	if (nRet != IMV_OK)
	{
		printf("Get device info failed! ErrorCode[%d]\n", ret);
		return grabStrartegySequential;
	}

	printf("\n--------------------------------------------\n");
	printf("\t0.grabStrartegySequential\n");
	printf("\t1.grabStrartegyLatestImage\n");
	// 只有Gige设置支持下一帧取流模式 
	// Waiting for next image(GigE only)
	if (dvInfo.nCameraType == typeGigeCamera)
	{
		printf("\t2.grabStrartegyUpcomingImage\n");
	}
	printf("--------------------------------------------\n");

	strategyCnt = (dvInfo.nCameraType == typeGigeCamera) ? 3 : 2;
	...
	while (1)
	{
		...
		{
			inputIndex = atoi(pTrimStr);
			if ((inputIndex >= 0) && (inputIndex < (int)strategyCnt))
			{
				break;
			}
		}

		printf("Input invalid! Please select the grab strategy index: ");
	}

	return (IMV_EGrabStrategy)inputIndex;
}
...
int main()
{
	...
	IMV_EGrabStrategy grabStrategy = grabStrartegyUndefined;
	...
	do
	{
		...
		// 选择拉流策略
		// Select grab strategy
		grabStrategy = selectGrabStrategy(devHandle);
		if (grabStrategy == grabStrartegyUndefined)
		{
			printf("select grabStrategy failed!\n");
			break;
		}

		// 开始拉流 
		// Start grabbing 
		ret = IMV_StartGrabbingEx(devHandle, 0, grabStrategy);
		if (IMV_OK != ret)
		{
