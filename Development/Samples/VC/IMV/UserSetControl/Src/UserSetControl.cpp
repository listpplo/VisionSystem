...
/// \~chinese
/// \brief 保存和加载相机配置示例
/// \example UserSetControl.cpp
/// \~english
/// \brief save and load camera configuration sample
/// \example UserSetControl.cpp
...
// 本程序演示了保存和设置相机配置
// This program shows how to save and set camera configuration
//**********************************************************************
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
static int restoreDefault(IMV_HANDLE devHandle)
{
	int ret = IMV_OK;

	//1、选择默认配置为当前配置
	//1、Select the default configuration as the current configuration
	ret = IMV_SetEnumFeatureSymbol(devHandle, "UserSetSelector", "Default");
	if (IMV_OK != ret)
	{
		printf("Set UserSetSelector feature value failed! ErrorCode[%d]\n", ret);
		return ret;
	}

	//2、加载默认配置
	//2、Load the default configuration
	ret = IMV_ExecuteCommandFeature(devHandle, "UserSetLoad");
	if (IMV_OK != ret)
	{
		printf("Excute UserSetLoad property fail! ErrorCode[%d]\n", ret);
		return ret;
	}

	//3、设置默认配置为下次相机启动时使用的默认配置
	//3、Set the default configuration as the default configuration when the camera is started
	ret = IMV_SetEnumFeatureSymbol(devHandle, "UserSetDefault", "Default");
	if (IMV_OK != ret)
	{
		printf("Set UserSetDefault feature value failed! ErrorCode[%d]\n", ret);
		return ret;
	}
...
static int saveUserConfiguration(IMV_HANDLE devHandle)
{
	int ret = IMV_OK;

	//1、选择当前配置为UserSet1
	//1、Select the UserSet1 configuration as the current configuration
	ret = IMV_SetEnumFeatureSymbol(devHandle, "UserSetSelector", "UserSet1");
	if (IMV_OK != ret)
	{
		printf("Set UserSetSelector feature value failed! ErrorCode[%d]\n", ret);
		return ret;
	...
	ret = IMV_ExecuteCommandFeature(devHandle, "UserSetSave");
	if (IMV_OK != ret)
	{
		printf("Excute UserSetSave property fail! ErrorCode[%d]\n", ret);
		return ret;
	}
...
static int LoadUserConfiguration(IMV_HANDLE devHandle)
{
	int ret = IMV_OK;
	...
	ret = IMV_SetEnumFeatureSymbol(devHandle, "UserSetDefault", "UserSet1");
	if (IMV_OK != ret)
	{
		printf("Set UserSetDefault feature value failed! ErrorCode[%d]\n", ret);
		return ret;
...
int main()
{
	...
	do
	{
		...
		ret = saveUserConfiguration(devHandle);
		if (IMV_OK != ret)
		{
			printf("Save user set failed!\n");
			break;
		}

		printf("Save configuration to userSet1 successfully...\n");

		// 加载相机当前配置
		// Load current camera configuration
		ret = LoadUserConfiguration(devHandle);
		if (IMV_OK != ret)
		{
			printf("Load current set failed!\n");
			break;
		}

		printf("Load configuration from userSet1 successfully...\n");
