...
static int selectSaveFormat(void)
{
	char inputStr[256];
	char* pTrimStr;
	int inputIndex = -1;
	...
	IMV_ESaveFileType saveFormat = typeSaveBmp;
	char* find = NULL;
	...
	switch (inputIndex)
	{
	case 0: saveFormat = typeSaveBmp; break;
	case 1: saveFormat = typeSaveJpeg; break;
	case 2: saveFormat = typeSavePng; break;
	case 3: saveFormat = typeSaveTiff; break;
	default: saveFormat = typeSaveBmp; break;
	}
...
int main()
{
	...
	do
	{
		...
		// 选择保存的图片格式
		// Select target format for image save 
		saveFormat = (IMV_ESaveFileType)selectSaveFormat();
		...
		if (typeSaveBmp == saveImageToFileParam.eImageType)
		{
			sprintf_s(saveImageToFileParam.pImagePath, 256, "Image_w%d_h%d_fn%03llu.bmp", saveImageToFileParam.nWidth, saveImageToFileParam.nHeight, frame.frameInfo.blockId);
		}
		else if (typeSaveJpeg == saveImageToFileParam.eImageType)
		{
			saveImageToFileParam.nQuality = 90;
			sprintf_s(saveImageToFileParam.pImagePath, 256, "Image_w%d_h%d_fn%03llu.jpg", saveImageToFileParam.nWidth, saveImageToFileParam.nHeight, frame.frameInfo.blockId);
		}
		else if (typeSavePng == saveImageToFileParam.eImageType)
		{
			saveImageToFileParam.nQuality = 8;
			sprintf_s(saveImageToFileParam.pImagePath, 256, "Image_w%d_h%d_fn%03llu.png", saveImageToFileParam.nWidth, saveImageToFileParam.nHeight, frame.frameInfo.blockId);
		}
		else if (typeSaveTiff == saveImageToFileParam.eImageType)
		{
			sprintf_s(saveImageToFileParam.pImagePath, 256, "Image_w%d_h%d_fn%03llu.tif", saveImageToFileParam.nWidth, saveImageToFileParam.nHeight, frame.frameInfo.blockId);
		}

		// 保存图片
		// Save image
		ret = IMV_SaveImageToFile(devHandle, &saveImageToFileParam);
		if (IMV_OK != ret)
		{
	...
	if (saveImageToFileParam.pImagePath)
	{
		free(saveImageToFileParam.pImagePath);
		saveImageToFileParam.pImagePath = NULL;
	}
