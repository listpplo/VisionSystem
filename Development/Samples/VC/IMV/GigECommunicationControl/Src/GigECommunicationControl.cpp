...
/// \~chinese
/// \brief GigE传输控制示例
/// \example GigECommunicationControl.cpp
/// \~english
/// \brief GigE communication control sample
/// \example GigECommunicationControl.cpp
...
int main()
{
	...
	do
	{
		...
		ret = IMV_GIGE_SetAnswerTimeout(devHandle, 1000);
		if (IMV_OK != ret)
		{
			printf("Set gige answer timeout failed! ErrorCode[%d]\n", ret);
			break;
		}
		...
		if (IMV_OK != ret)
		{
			printf("Set gige inter packet timeout failed! ErrorCode[%d]\n", ret);
			break;
		}

		// 设置单次重传最大包个数,仅对GigE设备有效
		// Set the single resend maximum packet number, only for Gige device
		ret = IMV_GIGE_SetSingleResendMaxPacketNum(devHandle, 50);
		if (IMV_OK != ret)
		{
			printf("Set gige sing resend max packet number failed! ErrorCode[%d]\n", ret);
			break;
		}

		// 设置同一帧最大丢包的数量,仅对GigE设备有效
		// Set the maximum lost packet number, only for Gige device
		ret = IMV_GIGE_SetMaxLostPacketNum(devHandle, 80);
		if (IMV_OK != ret)
		{
			printf("Set gige max lost packet number failed! ErrorCode[%d]\n", ret);
			break;
		}
