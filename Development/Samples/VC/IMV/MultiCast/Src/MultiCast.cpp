...
/// \~english
/// \brief Multicast function sample
/// \example MultiCast.cpp
...
int main()
{
	...
	do
	{
		...
		printf("Select Control Mode(c) or Monitor Mode(m).\n");
		char mode = '\0';
		do
		{
			scanf("%c", &mode);
		} while ((mode != 'c') && (mode != 'C') && (mode != 'm') && (mode != 'M'));
		getchar();
		
		bool isControlMode = (mode == 'c') || (mode == 'C');

		// 打开相机 
		// Open camera 
		if (isControlMode)
		{
			ret = IMV_OpenEx(devHandle, accessPermissionControl);
		}
		else
		{
			ret = IMV_OpenEx(devHandle, accessPermissionMonitor);
		}
		if (IMV_OK != ret)
		{
		...
		IMV_TRANSMISSION_TYPE transmissionType;
		memset(&transmissionType, 0, sizeof(transmissionType));

		transmissionType.eTransmissionType = transTypeMulticast;
		transmissionType.nDesIp = nIp;
		transmissionType.nDesPort = 1042;

		// 设置传输类型
		// Set transmission type
		ret = IMV_GIGE_SetTransmissionType(devHandle, transmissionType);
		if (IMV_OK != ret)
		{
			printf("set TransmissionType failed! ErrorCode[%d]\n", ret);
			break;
		}
		...
		printf("Press enter key to stop program.\n");
		getchar();
