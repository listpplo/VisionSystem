...
/// \~chinese
/// \brief 流统计示例
/// \example StreamStatistics.cpp
/// \~english
/// \brief stream statistics sample
/// \example StreamStatistics.cpp
...
static void displayStreamStatsInfo(IMV_FG_StreamStatisticsInfo streamStatsInfo)
{
	switch (streamStatsInfo.nDeviceType)
	{
	case IMV_FG_EDeviceType::IMV_FG_TYPE_CL_DEVICE:
		printf("clStatisticsInfo:imageError[%d]. lostPacketBlock[%d]. imageReceived[%d]. fps[%f]. bandwidth[%f]. \r\n",
			streamStatsInfo.clStatisticsInfo.imageError, streamStatsInfo.clStatisticsInfo.lostPacketBlock, streamStatsInfo.clStatisticsInfo.imageReceived,
			streamStatsInfo.clStatisticsInfo.fps, streamStatsInfo.clStatisticsInfo.bandwidth);
		break;
	case IMV_FG_EDeviceType::IMV_FG_TYPE_CXP_DEVICE:
		break;
	case IMV_FG_EDeviceType::IMV_FG_TYPE_GIGE_DEVICE:
		printf("gigeStatisticsInfo:imageError[%d]. lostPacketBlock[%d]. imageReceived[%d]. fps[%f]. bandwidth[%f]. \r\n",
			streamStatsInfo.gigeStatisticsInfo.imageError, streamStatsInfo.gigeStatisticsInfo.lostPacketBlock, streamStatsInfo.gigeStatisticsInfo.imageReceived,
			streamStatsInfo.gigeStatisticsInfo.fps, streamStatsInfo.gigeStatisticsInfo.bandwidth);
		break;
	case IMV_FG_EDeviceType::IMV_FG_TYPE_U3V_DEVICE:
		printf("u3vStatisticsInfo:imageError[%d]. lostPacketBlock[%d]. imageReceived[%d]. fps[%f]. bandwidth[%f]. \r\n",
			streamStatsInfo.u3vStatisticsInfo.imageError, streamStatsInfo.u3vStatisticsInfo.lostPacketBlock, streamStatsInfo.u3vStatisticsInfo.imageReceived,
			streamStatsInfo.u3vStatisticsInfo.fps, streamStatsInfo.u3vStatisticsInfo.bandwidth);
		break;
	default:
...
int main()
{
	...
	do
	{
		...
		nRet = IMV_FG_ResetStatisticsInfo(ifHandle);
		if (IMV_FG_OK != nRet)
		{
			printf("Reset statistics info failed! ErrorCode[%d]\n", nRet);
			break;
		}
		...
		// 重置流统计信息
		// Reset stream statistics infomation
		IMV_FG_StreamStatisticsInfo streamStatsInfo;
		nRet = IMV_FG_GetStatisticsInfo(ifHandle, &streamStatsInfo);
		if (IMV_FG_OK != nRet)
		{
			printf("Reset statistics info failed! ErrorCode[%d]\n", nRet);
			break;
		...
		//打印流统计信息
		//print stream statistics infomation
		displayStreamStatsInfo(streamStatsInfo);
