...
/// \~chinese
/// \brief 获取触发计数统计示例
/// \example FrameTriggerCount.cpp
/// \~english
/// \brief get trigger count sample
/// \example FrameTriggerCount.cpp
...
{
	int ret = IMV_FG_OK;
	IMV_FG_DEV_HANDLE camHandle = (IMV_FG_DEV_HANDLE)pUserData;
	if (NULL == camHandle)
	{
		return 0;
	}
...
{
	int ret = IMV_FG_OK;
	IMV_FG_DEV_HANDLE camHandle = (IMV_FG_DEV_HANDLE)pUser;

	if (camHandle == NULL)
	{
		printf("camHandle is NULL\n");
		return;
	}
	...
	ret = IMV_FG_GetIntFeatureValue(camHandle, "FrameTriggerCount", &frameTriggerCount);
	if (IMV_FG_OK != ret)
	{
	...
	ret = IMV_FG_GetIntFeatureValue(camHandle, "FrameTriggerLostCount", &frameTriggerLostCount);
	if (IMV_FG_OK != ret)
	{
	...
	printf("Frame blockId = %llu, FrameTriggerCount = %lld, FrameTriggerLostCount = %lld\n",
		pFrame->frameInfo.blockId, frameTriggerCount, frameTriggerLostCount);

	return;
}

static int setSoftTriggerConf(IMV_FG_IF_HANDLE ifHandle, IMV_FG_DEV_HANDLE camHandle)
{
	int ret = IMV_FG_OK;

	// 设置触发源为软触发 
	// Set trigger source to Software 
	ret = IMV_FG_SetEnumFeatureSymbol(camHandle, "TriggerSource", "Software");
	if (IMV_FG_OK != ret)
	{
	...
	// 触发计数清0 
	// Clear trigger count
	ret = IMV_FG_ExecuteCommandFeature(camHandle, "FrameTriggerCountReset");
	if (IMV_FG_OK != ret)
	{
...
int main()
{
	...
	do
	{
		...
		nRet = IMV_FG_AttachGrabbing(ifHandle, onGetFrame, devHandle);
		if (IMV_FG_OK != nRet)
		{
		...
		nRet = setSoftTriggerConf(ifHandle, devHandle);
		if (IMV_FG_OK != nRet)
		{
