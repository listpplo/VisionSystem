...
/// \~english
/// \brief stream grabbing (callback) sample
/// \example GrabCallback.cpp
...
int main()
{
	...
	do
	{
		...
		nRet = IMV_FG_EnumInterface(typeCLInterface | typeCXPInterface, &interfaceList);
		if (IMV_FG_OK != nRet)
		{
		...
		//枚举相机设备
		// discover camera 
		nRet = IMV_FG_EnumDevices(typeCLInterface | typeCXPInterface, &camListPtr);
		if (IMV_FG_OK != nRet)
		{
