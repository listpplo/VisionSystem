...
/// \~chinese
/// \brief 帧克隆处理示例
/// \example FrameClone.cpp
/// \~english
/// \brief frame clone sample
/// \example FrameClone.cpp
...
{
	int ret = IMV_FG_OK;
	IMV_FG_IF_HANDLE camHandle = (IMV_FG_IF_HANDLE)pUser;
	IMV_FG_Frame frameClone;
	...
	ret = IMV_FG_CloneFrame(camHandle, pFrame, &frameClone);
	if (IMV_FG_OK != ret)
	{
		printf("Clone frame failed! ErrorCode[%d]\n", ret);
		return;
	}

	printf("Get clone frame blockId = %llu\n\n", frameClone.frameInfo.blockId);

	ret = IMV_FG_ReleaseFrame(camHandle, &frameClone);
	if (IMV_FG_OK != ret)
	{
