...
static int setLineTriggerConf(IMV_FG_IF_HANDLE	ifHandle,IMV_FG_DEV_HANDLE	camHandle)
{
	...
	// 设置采集卡触发源为外部触发 
	// Set capture baord trigger source to Line1 
	ret = IMV_FG_SetEnumFeatureSymbol(ifHandle, "CC1", "ExternalTrigger1");
	if (IMV_FG_OK != ret)
	{
	...
	// 设置相机触发器 
	// Set camera trigger selector to FrameStart 
	...
	ret = IMV_FG_SetEnumFeatureSymbol(camHandle, "TriggerActivation", "RisingEdge");
	if (IMV_FG_OK != ret)
	{
